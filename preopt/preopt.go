...
func main() {
	...
	filename := flag.String("file", "sample.xyz", "The XYZ file containing the coordinates for the system.")
	...
	optimize := flag.Bool("opt", true, "Whether to optimize or run an SP calculation.")
	...
	calc.Job = &qm.Job{Opti: true} //for the preeliminar calculation we always optimize, regarless of the user's input, because it's, well, a preoptimization.
	if calc.Job.Opti {
		calc.SCFTightness = 1
	}
	calc.Dispersion = "D3"
	...
	chem.XYZFileWrite("sample_preopt.xyz", ncoords, mol)
